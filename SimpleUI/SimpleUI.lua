...
local function check_components()
  ...
  local clicked_left  = love.mouse.isDown(1)
  local clicked_right = love.mouse.isDown(2)
  local draw 
  for i =1 ,#components do
   focused , draw = components[i]:update(clicked_left,x,y,focused,clicked_right)
...
function ui.init()
    ...
    main_canvas = love.graphics.newCanvas(love.graphics.getWidth(),love.graphics.getHeight())
     ...
     if major == 11 then
      settings.button = col_11
     else
      settings.button = col_pre11
     end
